...
type BinTree<'a> =
    { btValue: 'a
      btLeft: BinTree<'a> option
      btRight: BinTree<'a> option }
...
type Zipper<'a> =
    { zValue: 'a
      zLeft: BinTree<'a> option
      zRight: BinTree<'a> option
...
let zipper value left right crumbs =
    { zValue = value
      zLeft = left
      zRight = right
      zCrumbs = crumbs }
...
let toTree z =
    ...
    tree z.zValue z.zLeft z.zRight |> loop z.zCrumbs
...
let left z =
    ...
    | None -> None
    | Some l ->
        zipper l.btValue l.btLeft l.btRight ((LeftCrumb(z.zValue, z.zRight)) :: z.zCrumbs)
        |> Some
...
let right z =
    ...
    | None -> None
    | Some r ->
        zipper r.btValue r.btLeft r.btRight ((RightCrumb(z.zValue, z.zLeft)) :: z.zCrumbs)
        |> Some
...
let up z =
    ...
    | [] -> None
    | (LeftCrumb (tv, tr)) :: cs ->
        zipper tv (tree z.zValue z.zLeft z.zRight |> Some) tr cs
        |> Some
    | (RightCrumb (tv, tl)) :: cs ->
        zipper tv tl (tree z.zValue z.zLeft z.zRight |> Some) cs
        |> Some
