...
let private playedTiles board =
    ...
    |> Seq.groupBy fst
    |> Seq.map (fun (c, sp) -> (c, sp |> Seq.map snd |> Seq.toList))
    |> Map.ofSeq
...
let winner (board: string list) =
    ...
    let whiteGame =
        ...
        && findRoute (List.filter (fun (_, y) -> y = 0) marked.['O']) (fun (_, y) -> y = board.Length - 1) marked.['O']
